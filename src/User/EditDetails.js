import React, { useState, useEffect } from "react";
import Box from "@mui/material/Box";
import Button from "@mui/material/Button";
import Typography from "@mui/material/Typography";
import Modal from "@mui/material/Modal";
import { useForm, Controller, useFieldArray } from "react-hook-form";
import { Divider, TextField } from "@mui/material";
import Paper from "@mui/material/Paper";
import Grid from "@mui/material/Grid";
import { styled } from "@mui/material/styles";
import CustomTextField from "../components/TextField";

import MenuItem from "@mui/material/MenuItem";
import FormControl from "@mui/material/FormControl";
import Select, { SelectChangeEvent } from "@mui/material/Select";
import Alert from "@mui/material/Alert";
import AlertTitle from "@mui/material/AlertTitle";
import Stack from "@mui/material/Stack";
import { Modal_OpenClose } from "../store/Modal";
import EditProfile from './UserCall/EditProfile'
import LinearProgress from "@mui/material/LinearProgress";
// import {
//   handleModal,
//   selectDestination,
//   addDestinationID,
//   addDestinationType,
// } from "../store/Module";
import { pink, red } from "@mui/material/colors";
import { useDispatch, useSelector } from "react-redux";

import { OnlinePredictionSharp } from "@mui/icons-material";
import { resetWarningCache } from "prop-types";
import UpdateProfile from "./UserCall/UpdatingProfile";
import { Navigate, useNavigate } from "react-router-dom";


var a;
export default function EditDetails(props) {
  const Dest_Type = useSelector((state) => state.Dest.Destination_type);


  const [ExtensionShow, setExtensionShow] = React.useState(false);
  const [Data, setData] = useState({});
  const [useTime, setuseTime] = useState(false);

  const [selectMOH, setSelectMOH] = React.useState(false); // To check if MOH is selected
  const [InputTimeout, setInputTimeout] = React.useState(0); //InputTimeout Select

  const [Error, setError] = useState(null);
  const [Success, setSuccess] = useState(null);
  const [CreateData, setCreateData] = useState([]); //For Ring Plan
  const [MOH, setMOH] = useState([]); //For MOH
  const [openForm, setOpenForm] = useState(false);
  const [loading, setLoading] = useState(false);
  // const [OpenDest, setOpenDest] = useState(false);

  //STATES to store related data of API RESPONSE
  const [Syst_SO, setSSO] = useState(null);
  const [Input_TO, setITO] = useState(null);
  const [Input_LO, setILO] = useState(null);
  const [EditData, setEditData] = useState('');
  
  const [selectedFile, setSelectedFile] = useState(null);
  const accent = pink.A200;

  const [buttonName, setButtonName] = useState("Play");
//'https://ui-avatars.com/api/?name=f+t&color=7F9CF5&background=EBF4FF'
  const [image, setImage] = useState();
  const [audioName, setAudioName] = useState(); // To send to axios
 
  const userID = localStorage.getItem("userID");
  // console.log('userID--------',userID)
  const dispatch = useDispatch();
  const navigate = useNavigate()
// console.log('EditData?.user_name ---' ,  EditData?.user_name )
const preLoaderValues = {
  username: EditData ? EditData?.user_name : "",
  name: EditData?.name || "",
  password: "",
  confirmPass: "",
  phone: EditData?.phone || "",
  email: EditData?.email || "",
}
useEffect(() => {
  reset(preLoaderValues)
}, [EditData?.user_name])

  const {
    handleSubmit,
    control,
    register,
    reset,
    setValue,

    formState: { errors },
  } = useForm({
    defaultValues: preLoaderValues
  });

  const style = {
    position: "absolute",
    left: "50%",
    right: "50%",
    top: "50%",
    // bottom: "50%",
    transform: "translate(-50%, -50%)",
    width: "60%",
    bgcolor: "background.paper",
    boxShadow: 24,
  };

  const addFile = (e) => {
    console.log("IMAGE", e.target.files[0]);
    if (e.target.files[0]) {
      setImage(URL.createObjectURL(e.target.files[0]));
      
      setAudioName(e.target.files[0]);
    }
  };

  const RemoveFile = () => {
    // console.log("AUDIO ", audio);
    setImage("");
  };
  useEffect(() => {
    setOpenForm(true);
  }, []);

  const header = {
    bgcolor: "#8dc63f",
    pl: 2,
    py: 0.51,
  };

  const onSubmit = ({
    username,
    name,
    password,
    confirmPass,
    phone,
    email,
  }) => {
    setExtensionShow(true);
    var bodyFormData = new FormData();
    console.log("username", username);
    console.log("password", password);

    console.log("confirmPass", confirmPass);

    console.log("phone", phone);

    console.log("email", email);
    console.log('audioName.name----------' , audioName.name)
    console.log("IMAGE PATH", { image, audioName });

    bodyFormData.append("id", userID);
    
    bodyFormData.append("name", name);
    bodyFormData.append("password", password);
    // bodyFormData.append("confirmPass", confirmPass);
    bodyFormData.append("phone", phone);
    bodyFormData.append("email", email);
    bodyFormData.append("password_confirmation", 'admin');
    // bodyFormData.append("photo", audioName);
    
    setData(bodyFormData);
  };
  const succesHandle = (CD) => {
    setSuccess(CD);
  };
  // console.log("SHOW", Error);

  const SSO = (CD) => {
    setSSO(CD);
  };
  const ILO = (CD) => {
    // console.log("SOWww", CD);
    setILO(CD);
  };
  const ITO = (CD) => {
    setITO(CD);
  };
  const handleDataGet = (CD) => {
    setCreateData(CD);
  };
  const handleMOH = (CD) => {
    setMOH(CD);
  };
  return (
    <div>
      {loading && (
        <Box sx={{ width: "100%" }}>
          <LinearProgress color="success" />
        </Box>
      )}
       {openForm && (
        <EditProfile parentCallback={(CD)=>setEditData(CD)}/>
      )} 

       {ExtensionShow && (
        <UpdateProfile
          body={Data}
          Success={succesHandle}
          Error={(err) => {
            setError(err);
          }}
          isLoading={(loading) => setLoading(loading)}
        />
      )} 

      <Box sx={style}>
        <Box sx={header}>
          <Typography
            id="modal-modal-title"
            variant="h6"
            component="h2"
            sx={{ color: "white", fontSize: 21 }}
          >
            Upload system Sound
          </Typography>
        </Box>

        {Success && (
          <>
            <Stack
              sx={{
                width: "100%",
                position: "absolute",
                zIndex: 2,
                top: 15,
                mt: 0,
                py: 0,
                display: "flex",
                justifyContent: "center",
                alignItems: "center",
              }}
              spacing={2}
            >
              <Alert
                variant="filled"
                severity="success"
                action={
                  <Button
                    onClick={() => navigate('/')}
                    color="inherit"
                    size="small"
                  >
                    OK
                  </Button>
                }
              >
                {/* <AlertTitle>Success</AlertTitle> */}
                <strong> {Success?.message}</strong>
              </Alert>
            </Stack>
          </>
        )}
        <Box>
          <Box sx={{ flexGrow: 1, p: 2 }}>
            <form onSubmit={handleSubmit(onSubmit)}>
              <Typography sx={{ color: "#8dc63f", fontSize: 20 }}>
                Edit Profile
              </Typography>
              <Divider sx={{ py: 0.5 }} />
              <Grid container sx={{ py: 1 }} spacing={2}>
                <Grid item xs={6}>
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "flex-start",
                      alignItems: "center",
                    }}
                  >
                    <label style={{ marginRight: 10 }} htmlFor="username">
                      Username
                    </label>

                    <Controller
                      control={control}
                      name="username"
                      // defaultValue=""
                      

                      render={({ field }) => (
                        <CustomTextField
                          // name="namefield"
                          inputProps={
                            { readOnly: true, }
                          }
                          // disabled
                          size="small"
                          id="username"
                          {...field}
                          
                          inputRef={field.ref}
                            defaultValue={EditData?.user_name }
                          error={Error?.name ? true : false}
                          helperText={Error?.name?.toString()}
                        />
                      )}
                    />
                  </div>
                </Grid>

                <Grid item xs={6}>
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "flex-start",
                      alignItems: "center",
                    }}
                  >
                    <label style={{ marginRight: 10 }} htmlFor="name">
                      Name
                    </label>

                    <Controller
                      control={control}
                      name="name"
                      // defaultValue=""

                      render={({ field }) => (
                        <CustomTextField
                          // name="namefield"
                          size="small"
                          id="name"
                          {...field}
                          label="Text field"
                          inputRef={field.ref}
                          //   defaultValue={EditedData?.List_Name}
                        
                        />
                      )}
                    />
                  </div>
                </Grid>

                <Grid item xs={6}>
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "flex-start",
                      alignItems: "center",
                    }}
                  >
                    <label style={{ marginRight: 10 }} htmlFor="password">
                      Password
                    </label>

                    <Controller
                      control={control}
                      name="password"
                      // defaultValue=""

                      render={({ field }) => (
                        <CustomTextField
                          // name="namefield"
                          size="small"
                          id="password"
                          {...field}
                          type="password"
                          inputRef={field.ref}
                          //   defaultValue={EditedData?.List_Name}
                          error={Error?.name ? true : false}
                          helperText={Error?.name?.toString()}
                        />
                      )}
                    />
                  </div>
                </Grid>

                <Grid item xs={6}>
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "flex-start",
                      alignItems: "center",
                    }}
                  >
                    <label style={{ marginRight: 10 }} htmlFor="confirmPass">
                      Confirm Password
                    </label>

                    <Controller
                      control={control}
                      name="confirmPass"
                      // defaultValue=""

                      render={({ field }) => (
                        <CustomTextField
                          // name="namefield"
                          size="small"
                          id="confirmPass"
                          {...field}
                          label="Text field"
                          type='password'
                          inputRef={field.ref}
                          //   defaultValue={EditedData?.List_Name}
                          // error={Error?.name ? true : false}
                          // helperText={Error?.name?.toString()}
                        />
                      )}
                    />
                  </div>
                </Grid>

                <Grid item xs={6}>
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "flex-start",
                      alignItems: "center",
                    }}
                  >
                    <label style={{ marginRight: 10 }} htmlFor="phone">
                      Phone
                    </label>

                    <Controller
                      control={control}
                      name="phone"
                      // defaultValue=""

                      render={({ field }) => (
                        <CustomTextField
                          // name="namefield"
                          size="small"
                          id="phone"
                          {...field}
                          // label="Text field"

                          inputRef={field.ref}
                          //   defaultValue={EditedData?.List_Name}
                          error={Error?.phone ? true : false}
                          helperText={Error?.phone?.toString()}
                        />
                      )}
                    />
                  </div>
                </Grid>

                <Grid item xs={6}>
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "flex-start",
                      alignItems: "center",
                    }}
                  >
                    <label style={{ marginRight: 10 }} htmlFor="email">
                      Email
                    </label>

                    <Controller
                      control={control}
                      name="email"
                      // defaultValue=""

                      render={({ field }) => (
                        <CustomTextField
                          // name="namefield"
                          size="small"
                          id="email"
                          {...field}
                          // label="Text field"

                          inputRef={field.ref}
                          //   defaultValue={EditedData?.List_Name}
                          error={Error?.email ? true : false}
                          helperText={Error?.email?.toString()}
                        />
                      )}
                    />
                  </div>
                </Grid>

             
                <Grid item xs={6}>
                  <div
                    style={{
                      display: "flex",
                      justifyContent: "flex-start",
                      alignItems: "center",
                    }}
                  >
                    <label htmlFor="btn-upload">Upload Image</label>
                    <input
                      type="file"
                      accept=".png,.jpg,.jpeg"
                      onChange={addFile}
                    />

                    <Button variant="contained" onClick={RemoveFile}>
                      Remove
                    </Button>
                  </div>
                  <div style={{ paddingLeft: 10, color: "red" }}>
                    {Error?.photo || ""}
                  </div>
                </Grid>
              </Grid>

              <Grid container sx={{ py: 1 }} spacing={2}>
                <Grid item xs={10} />

                <Grid item xs={2}>
                  <Button
                    type="submit"
                    sx={{ mx: 2, backgroundColor: "" }}
                    variant="contained"
                  >
                    UPDATE
                  </Button>
                </Grid>
              </Grid>
            </form>
          </Box>
        </Box>
      </Box>
    </div>
  );
}
